...
def store_root_cause(incident_id, root_causes):
    ...
    # Double-check description is clean text, not JSON
    if description.strip().startswith('[') or description.strip().startswith('{'):
        description = extract_clean_description({'description': description})
    ...
    try:
        table.update_item(
            ...
            ExpressionAttributeValues={
                ':rc': description[:200],  # Limit length
        ...
        try:
            ...
            if isinstance(parsed, list) and parsed:
                if isinstance(parsed[0], dict):
                    inner_desc = parsed[0].get('description', '')
                    if inner_desc and not inner_desc.strip().startswith('['):
                        return inner_desc[:150]
                    return str(parsed[0])[:150]
                return str(parsed[0])[:150]
            if isinstance(parsed, dict):
                inner_desc = parsed.get('description', '')
                if inner_desc and not inner_desc.strip().startswith('['):
                    return inner_desc[:150]
                return str(parsed)[:150]
        except:
            pass
        # Still looks like JSON but couldn't parse - extract text before first quote
        # Pattern: [{"description": "actual text here"
        import re
...
def infer_category(description):
    ...
    if any(w in desc_lower for w in ['capacity', 'scale', 'traffic', 'load', 'resource', 'memory', 'cpu', 'exhaustion']):
        return 'capacity'
    if any(w in desc_lower for w in ['slow', 'latency', 'performance', 'query', 'timeout', 'response time']):
        return 'performance'
    if any(w in desc_lower for w in ['config', 'setting', 'parameter', 'threshold', 'limit']):
        return 'configuration'
    if any(w in desc_lower for w in ['deploy', 'release', 'version', 'code change', 'update']):
        return 'deployment'
    if any(w in desc_lower for w in ['dependency', 'downstream', 'external', 'third-party', 'api call']):
        return 'dependency'
    return 'unknown'
...
- Include a category for each root cause

Return format:
[
  {{"description": "short plain text sentence", "confidence": 80, "category": "capacity|performance|configuration|deployment|dependency|unknown", "evidence": "what data supports this"}},
  {{"description": "another plain text sentence", "confidence": 60, "category": "capacity|performance|configuration|deployment|dependency|unknown", "evidence": "supporting data"}}
]

Categories:
- capacity: Resource exhaustion, scaling issues, traffic spikes
- performance: Slow queries, latency issues, inefficient code
- configuration: Misconfigurations, wrong settings, parameter issues
- deployment: Recent deployments, code changes, version issues
- dependency: External service failures, downstream issues
    ...
    try:
        ...
        if text.startswith('['):
            ...
            if start >= 0 and end > start:
                ...
                desc = text[:100]
                return [{'description': desc, 'confidence': 50, 'category': infer_category(desc), 'evidence': 'Could not parse structured response'}]
        
        # Validate and clean each cause - ALWAYS ensure category is set
        cleaned = []
        for cause in causes[:3]:
            if isinstance(cause, dict):
                        ...
                        if isinstance(inner, list) and inner and isinstance(inner[0], dict):
                            ...
                            for inner_cause in inner[:3]:
                                ...
                                if isinstance(inner_desc, str) and not inner_desc.strip().startswith('['):
                                    # ALWAYS set category
                                    cat = inner_cause.get('category', '')
                                    if not cat or cat == 'N/A' or cat == 'unknown':
                                        cat = infer_category(inner_desc)
                                    cleaned.append({
                ...
                if isinstance(desc, str) and not desc.strip().startswith('{') and 'Parse error' not in desc:
                    # ALWAYS ensure category is set - check for empty, None, N/A
                    cat = cause.get('category', '')
                    if not cat or cat == 'N/A' or cat == 'unknown' or cat == '':
        ...
        # Final pass - ensure ALL entries have valid categories
        for item in cleaned:
            if not item.get('category') or item.get('category') in ['N/A', '', None]:
                item['category'] = infer_category(item.get('description', ''))
        
        return cleaned if cleaned else [{'description': response_text[:100], 'confidence': 50, 'category': infer_category(response_text[:100]), 'evidence': 'Parse fallback'}]
    except Exception as e:
        ...
        desc = text[:100]
        return [{'description': desc, 'confidence': 50, 'category': infer_category(desc), 'evidence': f'Parse error: {str(e)[:50]}'}]
